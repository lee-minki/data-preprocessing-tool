...
### OPC 태그 로컬 캐시 (Mac 데스크톱)

자주 사용하는 OPC 태그의 히스토리를 `data_cache/<tag>.xlsx` 로 저장해
두면 다음부터 OPC를 다시 호출하지 않고 즉시 로드합니다 (2020년 이후
6년치도 즉시 로드).

- 태그 1개 = 파일 1개, 연도별 시트 (`2020`, `2021`, ...)
- 컬럼: `A=datetime`, `B=value`, `C=quality(선택)` — Excel에서 직접 열어
  편집·붙여넣기 가능
- OPC 탭의 `📦 캐시 우선 사용` 체크박스로 캐시 우선 동작 토글
- `자동 저장` 이 켜져 있으면 OPC에서 받아온 데이터를 자동으로 캐시에 병합
- 자세한 사용법: `data_cache/README.md` 참고

...
이 기능은 업로드한 전처리 대상 XLSX 데이터의 **좌측 첫 시간 컬럼**에 맞춰 OPC 태그 값을 내부 **임시 보조 컬럼(helper column)** 으로 붙여 조건/소팅에 활용하고, 최종 산출물 저장 시에는 helper 컬럼을 제거해 원본 템플릿 행/열 구조를 보존하는 기능이다.
...
- 최종 산출물에는 helper OPC 컬럼을 포함하지 않고, 원본 템플릿의 기존 행/열 구조를 임의로 바꾸지 않는다.
...
- 업로드 데이터의 좌측 첫 시간 컬럼을 시간축/조회 범위 기준으로 사용한다.
...
XLSX 업로드
  ...
  -> 좌측 첫 시간 컬럼 기준으로 __helper__ 컬럼을 내부 부착
...
- OPC helper 값 조회 기준은 자동 감지가 아니라 업로드 XLSX의 좌측 첫 컬럼이다.
- 시간 정규화/시간 재정렬은 기본 활성화되어 있으며, 조기경보 재업로드 템플릿의 시간축을 그대로 유지해야 하는 경우에는 사용자가 체크 해제한다.
- helper 컬럼은 화면/조건 계산에서만 쓰고 최종 다운로드에는 포함하지 않는다.
- 기존 원본 컬럼명과 순서는 export 시 원본 기준으로 유지한다.


### 13.6 검증으로 고정한 웹 동작

- 필터는 조건에 맞는 행만 남긴다. `= 1`은 1인 행만 남기고, `!= 1`은 1이 아닌 행만 남긴다.
- 이상값 처리 대상 컬럼은 기본 해제 상태이며, 이상값 처리가 켜져 있으면 1개 이상 선택해야 한다.
- 이상값 처리 대상 선택은 Simulation 대상에 자동 반영된다. 단, Simulation 대상을 사용자가 직접 수정한 뒤에는 그 선택을 유지한다.
- 시간 재정렬 시작 시간은 2분 배수로 자동 내림 보정한다. 예: `10:05:00` → `10:04:00`.
- 빈 헤더 열은 로드 시 제외해 Simulation/다운로드 파일에 빈 헤더 열과 값이 붙지 않도록 한다.
